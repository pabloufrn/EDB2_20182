...
class HashTable:
    ...
    def insert(self, val_):
        if(not isinstance(val_, Entry)):
            print("Essa HashTable só aceita valores do tipo Entry.")
            return

        if(self.load_factor() >= 1.0):
            self.rehash()

        index = self._hf(val_._key) % self._capacity
    ...
    def search(self, key_):
        ...
        index = self._hf(key_) % self._capacity
...
if __name__ == "__main__":
    ...
    ht = HashTable(hf)
    ht.insert(Entry(5, 5))
    ht.insert(Entry(16, 16))
    ht.insert(Entry(7, 7))
    ht.insert(Entry(12, 12))
